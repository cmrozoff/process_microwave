...
%
hr_end = str2num(fileNameShort(27:28));
mn_end = str2num(fileNameShort(29:30));
hr_full = hrS + mnS / 60.0;
hr_end_full = hr_end + mn_end / 60.0;
if hr_end_full < hr_full
    doy_end = doyS + 1;
else
    doy_end = doyS;
end
timeBase = datenum([yrS 1 1 0 0 0]);
timeBeg = timeBase + doyS -  1 + (hr_full / 24.0);
timeEnd = timeBase + doy_end - 1 + (hr_end_full / 24.0);
ntimes = length(bt85h(1,:));
dtime = (timeEnd - timeBeg) / (ntimes-1);
lonS = lon85;
latS = lat85;
btS = bt85h;
timeS = [timeBeg:dtime:timeEnd]';
%
% What is the starting time of the satellite swath?
%
inferior = min(timeS);
...
if ~isempty(indbt)
    ...
    for j = 1:n
        ...
        [arclen, az] = distance(interpLat, interpLon, ...
            squeeze(latS(90,:))', squeeze(lonS(90,:))');
