function [amsreData]=realTimeAMSREbasics(filename);
%
...
nparam = 7; %input('Number of parameters to write out or 0 to exit program. ');
...
elseif nparam == n_datasets
    ...
    temparm = [1 2 23 24 27 28 12]; % 92 variables are available
    sds_index = temparm;
...
amsreData.bt19V=double(data{3}');
amsreData.bt19H=double(data{4}');
amsreData.bt37V=double(data{5}');
amsreData.bt37H=double(data{6}');
amsreData.earthAzimuth=double(data{7}');
%
